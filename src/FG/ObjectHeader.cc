...
namespace FG {
  ...
  void ObjectHeader::instdata::clear() {
    str1 = "";
    str2 = "";
    objtype = "";
    instid = -1;
  ...
  void ObjectHeader::instdata::print() {
    printf("str1: %s\nstr2: %s\n", str1.c_str(), str2.c_str());
    printf("objtype: %s\ninstid: %i\n", objtype.c_str(), instid);
    printf("compname: %s\ncompinstid: %i\n", compname.c_str(), compinstid);
  }

  ObjectHeader::ObjectHeader(Reader& _reader): Saveable() {
    deserialize(_reader);
  }
  ...
  bool ObjectHeader::parse_inststr(const std::string& _str, bool _entity, ObjectHeader::instdata& _data) {
  ...
  void ObjectHeader::deserialize(Reader& _reader) {
    //TRACE;
  ...
  void ObjectHeader::deserializeCompDefs(Reader& _reader) {
    std::string a,b;
    int32_t n;
  ...
  std::string ObjectHeader::str_compbase() const {
    std::string rv;
